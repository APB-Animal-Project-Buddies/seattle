// Menus page — composed plant-based menu generator for restaurant kitchens.
// Pick cuisine(s), apply diet/mood filters, get a randomized N-course menu
// with pairings and per-plate costs. Reads window.APB_RECIPES (135 recipes)
// + window.CUISINE_META populated by /recipes/data/*.js + helpers.jsx.
...
const COURSE_PLANS = {
  3: [
    { key: 'starter',  label: 'Apertivo · Starter',     pickFrom: ['easy','quick'], courses: ['starter'] },
    { key: 'main',     label: 'Plato fuerte · Main',    pickFrom: ['hero'],          courses: ['main', 'showstopper'] },
    { key: 'dessert',  label: 'Dolce · Sweet finish',   pickFrom: ['any'],           courses: ['dessert'] },
  ],
  4: [
    { key: 'starter',  label: 'Apertivo · Starter',     pickFrom: ['easy','quick'], courses: ['starter'] },
    { key: 'second',   label: 'Secondo · Small plate',  pickFrom: ['easy'],          courses: ['starter', 'main'] },
    { key: 'main',     label: 'Plato fuerte · Main',    pickFrom: ['hero'],          courses: ['main', 'showstopper'] },
    { key: 'dessert',  label: 'Dolce · Sweet finish',   pickFrom: ['any'],           courses: ['dessert'] },
  ],
  5: [
    { key: 'amuse',    label: 'Amuse · One bite',       pickFrom: ['quick'],         courses: ['starter'] },
    { key: 'starter',  label: 'Apertivo · Starter',     pickFrom: ['easy'],          courses: ['starter'] },
    { key: 'second',   label: 'Secondo · Small plate',  pickFrom: ['easy'],          courses: ['starter', 'main'] },
    { key: 'main',     label: 'Plato fuerte · Main',    pickFrom: ['hero'],          courses: ['main', 'showstopper'] },
    { key: 'dessert',  label: 'Dolce · Sweet finish',   pickFrom: ['any'],           courses: ['dessert'] },
  ],
  7: [
    ...
    { key: 'starter',  label: 'Apertivo · Starter',     pickFrom: ['easy'],          courses: ['starter'] },
    { key: 'second',   label: 'Secondo · Small plate',  pickFrom: ['easy'],          courses: ['starter', 'main'] },
    { key: 'pasta',    label: 'Primi · Grain or pasta', pickFrom: ['any'],           courses: ['main'] },
    { key: 'main',     label: 'Plato fuerte · Main',    pickFrom: ['hero'],          courses: ['main', 'showstopper'] },
    { key: 'cheese',   label: 'Intermezzo · Palate',    pickFrom: ['easy'],          courses: ['starter'] },
    { key: 'dessert',  label: 'Dolce · Sweet finish',   pickFrom: ['any'],           courses: ['dessert'] },
  ],
};
...
// Diet filters read from r.allergens (gluten | nuts | soy | coconut),
// derived in /public/recipes/data/*.json. Hide recipes that contain any
...
const NAME_PARTS = {
  ...
  brazilian:      { a: ['Casa','Cantina','Mesa','Roça','Quinta','Bar'],               b: ['Verde','do Mar','do Cerrado','da Praia','do Pampa','Tropical'] },
  'fast-food':    { a: ['Drive-Thru','Counter','Quick Stop','Lunch Window','Pickup'], b: ['Verde','Cruelty-Free','Plant Lab','Heritage','Greens','Express'] },
};
...
const PAIRING_BY_CUISINE = {
  ...
  brazilian:      ['Caipirinha (NA)','Guaraná','Açaí soda','Mate','Coconut water'],
  'fast-food':    ['Cold brew','Lemonade','Cider','Hibiscus tonic','Sparkling water'],
};
...
function classify(recipe) {
  const prepM = parseInt(recipe.prep || recipe.time || '60', 10) || 60;
  return {
    quick: prepM <= 25,
    easy: (recipe.difficulty || 1) <= 2,
    hero: (recipe.difficulty || 1) >= 2 || (recipe.cost || 0) >= 3.5,
  };
}

function generateMenu(cuisineIds, courses, dietIds) {
  const isFusion = cuisineIds.length > 1;
  const all = window.APB_RECIPES || [];

  let pool = all.filter(r =>
    cuisineIds.includes('all') || cuisineIds.includes(r.cuisine)
  );
  ...
  function pickFor(course, courseIdx) {
    const eligibleCourses = course.courses;
    const isDessertSlot = eligibleCourses.includes('dessert');

    // 1. In-cuisine, in-course
    let candidates = pool.filter(r =>
      !used.has(r.id) &&
      (r.courses || []).some(c => eligibleCourses.includes(c))
    );

    // 2. Dessert slot fallback: cross-cuisine for any unused dessert
    if (candidates.length === 0 && isDessertSlot) {
      const allRecipes = (window.APB_RECIPES || []).filter(r => {
        for (const dietId of dietIds) {
          const diet = DIETS.find(d => d.id === dietId);
          if (diet && !diet.match(r)) return false;
        }
        return true;
      });
      candidates = allRecipes.filter(r =>
        !used.has(r.id) && (r.courses || []).includes('dessert')
      );
    }

    // 3. Non-dessert slot fallback: any in-cuisine NON-dessert recipe
    //    (never let a dessert sneak into a starter / main slot)
    if (candidates.length === 0 && !isDessertSlot) {
    ...
    // 4. Last resort: same-cuisine, not-yet-used, any course
    if (candidates.length === 0) {
      candidates = pool.filter(r => !used.has(r.id));
    }
    if (candidates.length === 0) candidates = [...pool];

    const preferredCuisine = isFusion
      ? cuisineRotation[courseIdx % cuisineRotation.length]
      : null;

    const roleHints = course.pickFrom;
  ...
  const cuisineDisplay = isFusion
    ? cuisineIds.map(id => window.CUISINE_META.find(c => c.id === id)?.name).join(' × ')
    : cuisineIds[0] === 'all' ? 'Cross-cultural' : window.CUISINE_META.find(c => c.id === cuisineIds[0]).name;

  const cuisineTagDisplay = isFusion
    ? 'Fusion · cruelty-free, restaurant-grade'
    : cuisineIds[0] === 'all' ? 'Cruelty-free, restaurant-grade' : window.CUISINE_META.find(c => c.id === cuisineIds[0]).tag;
...
function swapCourse(menu, courseIdx) {
  ...
  const all = window.APB_RECIPES || [];
  let pool = cuisineIds.includes('all') ? all : all.filter(r => cuisineIds.includes(r.cuisine));
...
// Same shape & styles as components.jsx:SubTabs in /recipes — inlined here
// because /menus doesn't load components.jsx.
...
function MenusHero() {
  const idx = window.APB_INDEX || {};
  const totalRecipes = idx.totalRecipes || (window.APB_RECIPES || []).length;
...
function Generator({ cuisines, courses, servings, diets, onCuisineToggle, onCourses, onServings, onDietToggle, onMood, onGenerate }) {
  ...
  const cuisineList = (window.CUISINE_META || []).filter(c => c.id !== 'all');
  return (
      ...
      <div className="generator-grid">
        <div className="gen-stack">
          ...
          <div className="gen-row" style={{ flexDirection: 'row', gap: 32, flexWrap: 'wrap' }}>
            ...
            <div className="gen-row" style={{ minWidth: 240 }}>
...
function MenuCard({ menu, servings, onSwap, fading }) {
  if (!menu) return null;
  const totalPerGuest = menu.courses.reduce((s, c) => s + (c.recipe.cost || 0), 0);
      ...
      <div className={"menu-card" + (fading ? ' fading' : '')}>
        ...
        <div className="menu-head">
          ...
          <div className="price-row">
            ...
            <div className="pi">
              <div className="v">{menu.courses.reduce((s, c) => s + (parseInt(c.recipe.prep || c.recipe.time || '0', 10) || 0), 0)}m</div>
...
function History({ items, activeId, onPick }) {
  ...
  return (
    <div className="history">
      {items.map(m => {
        const total = m.courses.reduce((s, c) => s + (c.recipe.cost || 0), 0);
        return (
  ...
  return (
    <div className="tips">
      ...
      <div className="tip">
        ...
        <p>Hover any course and tap "Swap course" — we'll find a new dish in the same role and cuisine. Other courses, costs, and pairings stay put.</p>
      </div>
      <div className="tip">
        <div className="num">iii.</div>
        <h5>Diet filters are honest</h5>
        <p>Each recipe is tagged with the allergens its base ingredients contain. Toggle Gluten-, Nut-, Soy-, or Coconut-free to hide anything that wouldn't survive a strict allergy table.</p>
      </div>
    </div>
  );
}
...
function App() {
  ...
  // After a new menu renders, scroll it into view "modal-style" so the
  // generator section slides out of frame and the composed menu is centered.
  ...
  // First menu after data loads
  useEffect(() => {
    if (ready && !menu) setMenu(generateMenu(['italian'], 4, []));
  ...
  function handleGenerate() {
    ...
    setTimeout(() => {
      ...
      setMenu(next);
      if (next) setHistory(prev => [next, ...prev].slice(0, 8));
      setFading(false);
      scrollMenuIntoView();
    }, 240);
  }
  ...
  // Auto-regen when cuisine / courses / diets change
  useEffect(() => {
    if (ready) setMenu(generateMenu(cuisines, courses, diets));
    // eslint-disable-next-line
  }, [cuisines.join(','), courses, diets.join(','), ready]);

  if (!ready) {
    return (
      <div className="empty-state">
        <h3>Recipe data loading…</h3>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      />
      <div ref={menuRef} style={{ scrollMarginTop: 24 }}>
